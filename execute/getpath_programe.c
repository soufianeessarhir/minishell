...
/*   Updated: 2024/08/22 17:31:07 by relamine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	char	*setup_default_env(t_env **env_lst, char ***env,
	char **path_of_program, t_norm free_list)
{
	char	**tmp;
	char	*pwd_env_i;
...
static int	handle_env_setup(char **path_of_program, t_env **env_lst,
	t_norm free_list, char ***env)
{
	int		env_i;
	char	*pwd;
	char	*tmp_pwd;
	...
	if (**env == NULL)
	{
		...
		ft_export_anything(ft_strjoin("PWD=", tmp_pwd, free_list.lst),
			free_list.l_gc, free_list.lst, env);
		chdir(tmp_pwd);
	}
	{
	...
	system_export_config("@path_of_program=",
		*path_of_program, env, free_list.lst);
	return (env_i);
}
...
int	setup_env_and_path(char ***env, t_gc **lst, t_gc **l_gc)
{
	...
	if (!path_of_program && !my_getenv("@path_of_program", env_lst))
	{
		...
		{
			initialize_shell_environment(path_of_program, l_gc, lst, env);
			return (1);
		}
	}
	return (env_i);
